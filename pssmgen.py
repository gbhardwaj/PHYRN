...
parser = OptionParser(description = "Submits job to make PSSMs", usage = "USAGE: python %prog -i input_fasta_file -d full_path_nr -n new_db_name -e e_value", epilog = desc)
...
#Generate job file and submit job as a task array on a Sun Grid Machine#

#job_file = open("job_file.sh","w")
#blastpgp_path = '/share/pattersonlab/programs/blast-2.2.25/bin/blastpgp'
...
###Generate a job .script file and run it###
job_file = open("job_file.script","w")
blastpgp_path = '/user/local/bin/blast-2.2.25/bin/blastpgp'
nr_path = options.database
sqvalue = 1
lim = num
for sqvalue in range (1,lim):
	print >> job_file,blastpgp_path+" "+"-d"+" "+nr_path+" "+"-i"+" "+db_name+str(sqvalue)+".fa"+" "+"-o"+" "+db_name+str(sqvalue)+".out"+" "+"-C"+" "+db_name+str(sqvalue)+".asnt"+" "+"-e"+" "+"1"+" "+"-m"+" "+"0"+" "+"-s"+" "+"T"+" "+"-a"+" "+"1"+" "+"-h"+" "+str(e_value)+" "+"-j"+" "+str(options.iter)+" "+"-M"+" "+"BLOSUM62"+" "+"-F"+" "+"F"+" "+"-u"+" "+"1"+" "+"-J"+" "+"T"

job_file.close()

#submit job script file

os.chmod("job_file.script",0777)
job_run_cmd = "./job_file.script"
os.system(job_run_cmd)
